...
export class ShoppingListComponent {
  ...
  formatQuantity(q: Quantity | undefined, role?: IngredientRole): string {
    if (!q) return "";
    switch (q.kind) {
      case "exact": return `${trim0(q.value)} ${q.unit ?? ""}`.trim();
      case "range": return `${trim0(q.min)}–${trim0(q.max)} ${q.unit ?? ""}`.trim();
      case "count": return `${trim0(q.value)} ${q.unit ?? ""}`.trim();
      case "unspecified": return role === "toTaste" ? "" : "n. B.";
      case "toTaste": return "";
    }
  }

  qtyValue(i: Ingredient): string { return qtyFromIngredient(i); }
  unitValue(i: Ingredient): string { return unitFromIngredient(i); }
}
